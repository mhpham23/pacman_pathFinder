...
def cornersHeuristic(state, problem):
    ...
    distance = 0
    path = ((0, 0), (0, 0), 0)
    for currentCorner in corners_none:
        for corners_selected in corners_none:
            if currentCorner != corners_selected:
                distance = abs(currentCorner[0] - corners_selected[0]) + abs(currentCorner[1] - corners_selected[1])
                if (path[2] < distance):
                    path = (currentCorner, corners_selected, distance)

    if (path[0] != path[1]):
        dist1 = abs(x - path[0][0]) + abs(y - path[0][1])
        dist2 = abs(x - path[1][0]) + abs(y - path[1][1])
...
def foodHeuristic(state, problem):
    ...
    if (path[0] != path[1]):
        dist1 = abs(position[0] - path[0][0]) + abs(position[1] - path[0][1])
        dist2 = abs(position[0] - path[1][0]) + abs(position[1] - path[1][1])
        distance = path[2] + min(dist1, dist2)
    elif len(foodGrid) == 1:
        distance = abs(position[0] - foodGrid[0][0]) + abs(position[1] - foodGrid[0][1])
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState):
        ...
        while not heap.isEmpty():
            ...
            if state not in node_visited:
                node_visited.append(state)
                for current_state, current_path, current_cost in problem.getSuccessors(state):
                    new_path = path + [current_path]
                    new_cost = cost + current_cost
                    new_state = (current_state, new_path, new_cost)
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state):
        ...
        "*** YOUR CODE HERE ***"
        goal_flag = self.food.asList().count((x,y))
        goal_flag == 1
        return goal_flag
